...
if [ ${?} -eq 1 ]; then 
  logger -p local0.info "ERROR: slack-adapter-watchdog.sh for ${DOTSERVICE} tried to run on a server that isn't a RHEL7 server."
  exit 1
fi
...
if [ ${?} -eq 0 ]; then
  ...
  logger -p local0.info "slack-adapter-watchdog.sh detected a disconnect.  Restarting ${DOTSERVICE}."
