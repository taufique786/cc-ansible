#!/bin/sh
#
# skynet-slack-adapter-watchdog.sh
#
# On occasion, hubot will disconnect from slack.  This is usually due
# to network conditions affecting connectivity to slack.  The skynet
...
# of that connector and restart the skynet service if it finds it disconnected.
...
if [ ${?} -eq 1 ]; then
   logger -p local0.info "skynet-slack-adapter-watchdog.sh tried to run on a server that isn't a RHEL7 server."
   exit 1
fi
...
#
systemctl status skynet.service -l | grep "Active: active (running)" > /dev/null 2>&1
if [ ${?} -eq 1 ]; then
   exit 0
fi

# Check to see if skynet.service status shows disconnected. If yes,
# set a flag for use later.
#
DISCONNECTED=0
systemctl status skynet.service -l | grep "Slack client closed, waiting for reconnect" > /dev/null 2>&1
if [ ${?} -eq 0 ]; then
   DISCONNECTED=1
fi


# Check to see if skynet.service status shows connected.
# If it does but also shows disconnected in the status, it is disconnected.
# If it just shows connected, exit.
#
systemctl status skynet.service -l | grep "Slack client now connected" > /dev/null 2>&1
...
# If you've made it this far, the slack connector is disconnected, skynet service
...
logger -p local0.info "skynet-slack-adapter-watchdog.sh detected a disconnect.  Restarting skynet.service."
systemctl restart skynet.service >/dev/null  2>&1
